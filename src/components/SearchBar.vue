  ...
  >
    ...
    <div v-if="showDropdown" class="search-dropdown">
      <div v-for="location in dropdownLocations" :key="location.mapbox_id">
        <button class="search-suggestion" @click="selectSuggestion(location)">
      ...
      <button
        ...
        @click="clearAndUpdateRecentLocations()"
      >
        <div class="suggested-location-name-row">
...
const dropdownLocations = computed(() => {
  return textInput.value.trim() ? suggestions.value : recentLocations.value;
}); // Only show recentLocations in dropdown if user is not typing
...
async function fetchIfRequiredRecentLocs() {
  ...
  } else if (actualCurrentUser.uid !== currentUserID) {
    // Seems like there was a change in user since we last fetched
    currentUserID = actualCurrentUser.uid;
    recentLocations.value = await dbGetUserRecentLocations(
      actualCurrentUser.uid,
    );
  }
}
...
async function dbGetUserRecentLocations(userID) {
  ...
  if (userID) {
    const userRecentLocationRef = dbRef(
      database,
      "users/" + userID + "/recent-locations",
    );
...
async function querySuggestions(query, proximity = undefined) {
  ...
  const params = new URLSearchParams({
    ...
    language: "en",
    types:
      "country,region,district,postcode,locality,place,neighborhood,address,poi,street",
    limit: 5,
...
function typingHandler() {
  ...
  typingTimeout = setTimeout(async () => {
    // Async because of querying for suggestions
...
async function selectSuggestion(suggestion) {
  ...
  if (!res_json["features"]) {
    console.error(
      "No 'features' key on MapBox Retrieve API after User selects a suggested location",
    );
    return;
  }
  ...
  const actualCurrentUser = getAuth().currentUser;
  if (actualCurrentUser) {
    // Updates the user's recent locations list if they are logged in
    if (actualCurrentUser.uid !== currentUserID) {
      currentUserID = null;
      console.error(
        "User Get/Set Mismatch (Seems like the user when we requested Recent Locations from is different from the user we are setting to now). Resetting currentUserID to null",
      );
      return;
    }
...
function addToRecentLocations(addedLocation) {
  ...
  const existingInd = recentLocationsArr.findIndex((location) => {
    return location["mapbox_id"] === addedLocation["mapbox_id"];
  }); // .value due to Deep Reactivity
  if (existingInd != -1) {
    recentLocationsArr.splice(existingInd, 1);
  }
  recentLocationsArr.unshift(addedLocation);
  recentLocationsArr.length = Math.min(
    recentLocationsArr.length,
    MAX_USER_RECENT_LOCATIONS,
  );
}
